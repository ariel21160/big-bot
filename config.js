...
module.exports = kconfig = async (kill, message) => {
	...
	const techapi = 'INSIRA UMA API DO SITE OPEN API I-TECH' // MUDE ISSO PARA UMA API DO SITE QUE TA ALI
		...
		//aiden
          	  if (chats.match(/Aiden/gi)) {
					console.log('Aiden?.')
					kill.reply(from, '*Alguien dijo Aiden :D?', id)	
				}

		if (chats.match(/Alguien dijo Aiden :D?/gi)) {
			kill.reply(from, 'A ti no te sale callate', id)	
		}

		//puta
		if (chats.match(/puta/gi)) {
			kill.reply(from, 'Exclamo la puta', id)	
		}

		if (chats.match(/Exclamo la puta/gi)) {
			kill.reply(from, 'Se que queres ser como yo, pero como lo puta que eres no te va a salir lol', id)	
		}
		//gay
		if (chats.match(/gey/gi)) {
			kill.reply(from, 'exclamo el gey', id)	
		}

		if (chats.match(/gei/gi)) {
			kill.reply(from, 'exclamo el gey', id)	
		}

		if (chats.match(/gay/gi)) {
			kill.reply(from, 'exclamo el gey', id)	
		}
        ...
        case 'stiker':
			...
			case 'asenso':
				if (!isBotGroupAdmins) return aruga.reply(from, 'ups.. no puedo asender si no soy admin', id)
	...
	case 'aidenhistory':
	await kill.sendFileFromUrl(from, 'https://r2---sn-x1x7zn7s.c.drive.google.com/videoplayback?expire=1611959207&ei=Z1MUYIqABe2K8gStzYmIAg&ip=181.94.232.145&cp=QVRGU0pfWFJOSFhPOkpDZ19vaGJyVERkcm5hU1RsQjVjSTJTSENNR3pfOWpMdkJBaUprZDhWT2g&id=c933855f7b833db7&itag=18&source=webdrive&requiressl=yes&mh=x6&mm=32&mn=sn-x1x7zn7s&ms=su&mv=m&mvi=2&pl=22&sc=yes&ttl=transient&susc=dr&driveid=1wzBof_M0s-CvdWmKbv_1YEhQf5rtHpIU&app=explorer&mime=video/mp4&vprv=1&prv=1&dur=175.403&lmt=1611941955081327&mt=1611944667&sparams=expire,ei,ip,cp,id,itag,source,requiressl,ttl,susc,driveid,app,mime,vprv,prv,dur,lmt&sig=AOq0QJ8wRQIgZyPueS7MRVQ7ENybeJCA5Fch04dgzgJhM7c9ch1qYOMCIQCE8aBruumP4qAbVmppa-NlJwMSGxaYf6jwk2X8JyvDUA==&lsparams=mh,mm,mn,ms,mv,mvi,pl,sc&lsig=AG3C_xAwRQIgX27v9OU2hZIH_W_Lp93Du0xJCzn5u7uprjvupH-9Ic4CIQDgdEmHFPPLpJwSJbXCxU7H9GWd0OIi8suibsJuT2Qr0w==&cpn=yc3Jx35rrByrVo1S&c=WEB_EMBEDDED_PLAYER&cver=20210126', '', '', id)
	break
...
if (isMedia) {
		...
		case 'legiao':
			...
			kill.sendLinkWithAutoPreview(from, ownerNumber, 'Genial que te hayas interesado en contactar a aiden!\nAqui esta su whats!', id)
			break
	...
	case 'twit':
            ...
            const twi = await axios.get(`http://api.arugaz.my.id/api/media/twvid?url=${body.slice(4)}`)
       ...
       case 'ig':
        ...
        case 'mp4':
           if (args.length == 0) return kill.reply(from, 'Lo uso incorretamente.', id)
            axios.get(`http://api.arugaz.my.id/api/media/ytvideo?url=${body.slice(5)}`)
            .then(async(rest) => {
        ...
        case 'video':
            if (args.length == 0) return kill.reply(from, 'Lo uso incorrectamente.', id)
            axios.get(`https://api.arugaz.my.id/api/media/ytsearch?query=${body.slice(7)}`)
            .then(async (res) => {
	...
	case 'neon':
		...
		kill.reply(from, mess.wait, id)
		await kill.sendFileFromUrl(from, `https://api.arugaz.my.id/api/textpro/neontext?text=${body.slice(4)}`, '', '', id)
		break 
			
	case '3d':
		if (args.length == 0) kill.reply(from, 'Pon un mensaje ahí!', id)
		kill.reply(from, mess.wait, id)
		await kill.sendFileFromUrl(from, `https://api.arugaz.my.id/api/textpro/text3d?text=${body.slice(4)}`, '', '', id)
		break 
	...
	case 'lava':
		if (args.length == 0) kill.reply(from, 'Pon un nombre ahí!', id)
		kill.reply(from, mess.wait, id)
		await kill.sendFileFromUrl(from, `https://api.arugaz.my.id/api/textpro/lavatext?text=${body.slice(8)}`, '', '', id)
		break
			
	case 'toxic':
		if (args.length == 0) kill.reply(from, 'Pon un nombre ahí!', id)
		kill.reply(from, mess.wait, id)
		await kill.sendFileFromUrl(from, `https://api.arugaz.my.id/api/textpro/toxictext?text=${body.slice(8)}`, '', '', id)
		break
    ...
    case 'pornhub':
            ...
            if (args.length >= 2) {
                ...
                await kill.sendFileFromUrl(from, `https://api.arugaz.my.id/api/textpro/pornhub?text1=${fison}&text2=${twoso}`, '', '', id)
            } else {
